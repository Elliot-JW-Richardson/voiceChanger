...
**Deliberate division of labor with a Voice's own `gain` (see ADR 0005 addendum):** real-hardware testing found that closing Magos's full measured loudness gap against reference audio via a large per-Voice `gain`/`distortion` boost caused real artifacts (rhythmic ticking, burst noise — see CLAUDE.md's Real-time performance notes, point 10, for the root cause). The chosen split: a Voice's own chain applies only a small, clean gain (enough to stop it being inaudibly quiet), and Master Volume — already global, already user-adjustable live, already built with proper limiting — is the intended control for reaching a comfortable final loudness, not something a Voice's chain should try to fully replace.
