...
### Vocoder Effect Step (v2, scoped — see ADR 0004)
A distinct Effect Step type from the rest of the palette: it combines a **carrier** (a sawtooth oscillator, pitch-tracked to follow the input voice's own detected pitch in real time) with the input voice's own **modulator envelope** (its spectral/formant shape, extracted via a filter bank + envelope followers and imposed onto the carrier). This is what makes a vocoder intelligibly "speak" rather than just buzz — the classic talkbox/vocoder technique. Chosen after spectral analysis of a user-provided reference track (see ADR 0004) showed a melody-tracking, harmonically-rich low fundamental — not the fixed-carrier sidebands a simple ring-mod tune would produce. Also directly targets the "mumbly" quality naive pitch-shift causes, since it explicitly preserves formants rather than smearing them. Built in stages (see SLICES.md's Vocoder Band): pitch tracking, a pitch-tracked sawtooth carrier alone (an audible milestone in its own right), then true envelope-shaped vocoding, then voiced/unvoiced handling so unpitched consonants use a noise carrier instead of an incorrectly tonal sawtooth.
