...
9. **`distortion`/`limiter` Effect Steps added (real-hardware feedback: retuned Magos was "much better... but lacking richness") — two TEST Voices (`voices/magos-distortion-moderate.yaml`/`-aggressive.yaml`, `test: true`, not shipped as real Magos yet) chain 6 Effect Steps, the heaviest configuration benchmarked so far.** Across six independent 300-call runs against the aggressive variant (drive_db=45) plus `ApplyMasterVolume` at 150%: four showed 0/300 over budget, two showed 1/300 (max ~152-155ms) — comfortably within this machine's documented jitter pattern, but a noticeably TIGHTER margin than point 8's 4-step chain. Not a blocker for A/B listening as a test Voice, but if this design is ever adopted into the real, shipped Magos, re-benchmark it specifically (per point 5) before trusting it live — it may be the first chain to actually need a `SAMPLE_RATE`/`BLOCKSIZE` retune.
