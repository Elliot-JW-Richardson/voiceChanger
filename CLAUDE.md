...
- Sample rate is fixed at 48000 Hz, mono (1 channel) — set as module-level constants in both `app.py` and `main.py`, currently duplicated between them rather than shared. **Block size differs between the two on purpose**: `app.py` uses 8192 (see the "Real-time performance" note below); `main.py` uses 1024, since it does no DSP and benefits from lower latency instead.
...
4. **Block size trades off latency against processing headroom.** `pedalboard.PitchShift` has a large, roughly fixed per-call cost (~50ms measured on the dev machine) that barely scales with block size, so a small block size just means that fixed cost blows the budget more badly. `BLOCKSIZE=8192` (~170ms budget, ~170ms one-way latency) gives that ~50ms cost a comfortable margin. If a new Effect Step type turns out to be similarly expensive, benchmark it the same way (call it in a loop, compare per-call time to `BLOCKSIZE / SAMPLE_RATE`) before assuming the existing block size has enough headroom — don't guess.
