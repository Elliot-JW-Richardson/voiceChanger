...
6. **Magos briefly used `vocoder`, then reverted.** Slice 36 re-tuned Magos to `vocoder → pitch_shift → bitcrush` (benchmarked comfortably within budget), but real-hardware listening found the vocoder destroyed voice intelligibility outright (see point 7's post-mortem below and ADR 0004), and a follow-up "mix a sawtooth into the voice" experiment (`sawtooth_blend`) also failed on real hardware. Both were removed and Magos reverted to its original `pitch_shift → ring_mod → bitcrush` chain — the current, and only, shipped chain that's actually confirmed to sound like a voice. The `vocoder` Effect Step itself still exists in `voice_engine/engine.py` (no shipped Voice currently uses it) in case a future, more careful redesign wants to build on it. The current heaviest shipped chain is Magos's `pitch_shift → ring_mod → bitcrush` again — see point 5's original benchmark numbers for that chain (Slice 22).
